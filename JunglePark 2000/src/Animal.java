...
public class Animal implements ParkGUI {
   ...
   * tiger is being dragged (i.e. if its isDragging field is set to true). Also, this class perform
   * action of animals.
   */
  @Override
  public void draw() {
  ...
  public void mousePressed() {
    if (isMouseOver()) //use other method
      isDragging = true;
  }

  /**
   * Check mouse is released
   */
  @Override
  ...
  public PImage getImage() {
    return image; // for appearance of animals
  }
  ...
  public double distance(Animal otherAnimal) {
    ...
    // the distance between center of each animals
  }
  ...
  /**
   * Check another animal is near within range or not
   * 
   * @param otherAnimal reference to another animal
   * @param range       the standard distance between different animals
   * @return whether the animal is near or not
   */
  public boolean isClose(Animal otherAnimal, int range) {
    if (distance(otherAnimal) <= range) // the range can be changed
      return true;
    else
      return false;
  }
